 ...
 */
public final class WithVersionJsonDatabase<T> {
    private T data;
    private int schemaVersion;

    private WithVersionJsonDatabase(final T data, final int schemaVersion) {
        this.data = data;
     ...
     * @return the database
     */
    public static <T> WithVersionJsonDatabase<T> inMemory(T data, int schemaVersion) {
        return new WithVersionJsonDatabase<>(data, schemaVersion);
    }
     ...
     */
    public static <T> WithVersionJsonDatabase<T> fromFile(Gson gson, Path filePath, java.lang.reflect.Type dataType) throws IOException {
        try (final var reader = new FileReader(filePath.toFile())) {
            final var json = gson.fromJson(reader, JsonObject.class);
            return new WithVersionJsonDatabase<>(gson.fromJson(json.get("data"), dataType), json.get("schemaVersion").getAsInt());
        }
    }
    ...
    public JsonObject toJson(Gson gson) {
        ...
        obj.add("data", gson.fromJson(gson.toJson(data), JsonElement.class));
        return obj;
    }
