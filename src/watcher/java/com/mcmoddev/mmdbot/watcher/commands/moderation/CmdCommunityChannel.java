 ...
 *
 * TODO maybe move into The Commander?
 *
 * @author Unknown
 * @author Curle
 */
public final class CmdCommunityChannel extends SlashCommand {
    ...
    protected void execute(final SlashCommandEvent event) {
        ...
        final var categoryID = TheWatcher.getOldConfig().getCommunityChannelCategory();
        final var category = guild.getCategoryById(categoryID);
        if (categoryID == 0 || category == null) {
            TheWatcher.LOGGER.warn("Community channel category is incorrectly configured");
            event.reply("Community channel category is incorrectly configured. Please contact the bot maintainers.").queue();
            return;
        }

        final Set<Permission> ownerPermissions = TheWatcher.getOldConfig().getCommunityChannelOwnerPermissions();
        if (ownerPermissions.isEmpty()) {
            TheWatcher.LOGGER.warn("Community channel owner permissions is incorrectly configured");
        ...
        if (!diff.isEmpty()) {
            TheWatcher.LOGGER.warn("Cannot assign permissions to channel owner due to insufficient permissions: {}", diff);
        ...
        TheWatcher.LOGGER.info("Creating new community channel for {} ({}), named \"{}\" (command issued by {} ({}))",
        ...
        category.createTextChannel(channel)
            ...
            .flatMap(ch -> ch.sendMessage(new MessageBuilder()
                    ...
                    .appendFormat(" (which can be visited from the <#%s> channel),", TheWatcher.getOldConfig()
