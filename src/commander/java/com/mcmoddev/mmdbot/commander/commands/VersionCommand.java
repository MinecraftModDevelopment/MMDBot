...
public final class VersionCommand {
    ...
    public static final SlashCommand COMMAND = SlashCommandBuilder.builder()
        ...
        .children(
            ...
            SlashCommandBuilder.builder()
                .name("neoforge")
                .help("Get NeoForge versions for latest Minecraft version.")
                .options(new OptionData(OptionType.STRING, "version", "The version of Minecraft to check for."))
                .executes(event -> {
                    MinecraftForgeVersion latest;
                    OptionMapping version = event.getOption("version");
                    try {
                        ...
                        event.reply("Unable to get NeoForge versions.").setEphemeral(true).queue();
                        ex.printStackTrace();
                        return;
                    }

                    try {
                        final var latestNeoForgeVersion = latest.forgeVersion();
                        final var latestNeoForge = latestNeoForgeVersion.getLatest();
                        var recommendedNeoForge = latestNeoForgeVersion.getRecommended();
                        if (recommendedNeoForge == null) {
                            recommendedNeoForge = "none";
                        }
                        final var cgLink = "[Changelog](https://maven.neoforged.net/releases/net/neoforged/neoforge/%1$s/neoforge-%1$s-changelog.txt)"
                            .formatted(latest.forgeVersion().getLatest());
                        final var embed = new EmbedBuilder();

                        embed.setTitle(String.format("NeoForge Versions for MC %s", latest.mcVersion()));
                        embed.addField("Latest", latestNeoForge, true);
                        embed.addField("Recommended", recommendedNeoForge, true);
                        embed.setDescription(cgLink);
                        embed.setColor(Color.BLUE);
                        embed.setTimestamp(Instant.now());
                        event.replyEmbeds(embed.build()).mentionRepliedUser(false).queue();
