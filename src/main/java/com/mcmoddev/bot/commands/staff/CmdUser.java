...
public class CmdUser extends Command {
	...
	protected void execute(final CommandEvent event) {
		...
		//TODO This should not send if the command is used in the wrong channel.
		if (member == null) {
		...
		final long channelBotStuff = MMDBot.getConfig().getBotStuffChannelId();
		final long channelConsole = MMDBot.getConfig().getChannelIDConsole();
		if (channel.getIdLong() == channelBotStuff || channel.getIdLong() == channelConsole) {
			channel.sendMessage(embed.build()).queue();
		} else {
			channel.sendMessage("This command is channel locked.").queue();
		}
	}
